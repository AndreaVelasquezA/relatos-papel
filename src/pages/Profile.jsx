      ...
      <div className="profileContainer">
        ...
        <div className="profileCard">
          ...
          <div className="info">
            <p>📞 {user.phone}</p>
            <p>📍 {user.address}</p>
            <p>📅 Desde: {formatDate(user.joinedAt)}</p>
          </div>

          <button className="primaryBtn">✏️ Editar perfil</button>
        </div>
        ...
        <div className="statsGrid">
          ...
          <div className="statCard">
            <div className="statIcon">💰</div>
            <div className="statContent">
              <h3>{formatCurrency(user.stats?.totalSpent || 0)}</h3>
              <p>Total gastado</p>
            </div>
          </div>
          ...
          <div className="statCard">
            <div className="statIcon">⭐</div>
            <div className="statContent">
              <h3>{user.stats?.averageOrder?.toLocaleString() || 0}</h3>
        ...
        <div className="ordersSummary">
          ...
          {user.orders?.length === 0 ? (
            <div className="emptyOrders">
              ...
              <button className="secondaryBtn">Explorar libros</button>
            </div>
          ) : (
          ...
          <div className="modalContent" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="orderSummary">
              ...
              <div className="summaryRow">
                ...
                <span className="value">{selectedOrder.paymentMethod || "No especificado"}</span>
              </div>
              <div className="summaryRow">
                <span className="label">Costo de envío:</span>
                <span className="value">{formatCurrency(selectedOrder.shippingCost || 0)}</span>
              </div>
            </div>
              ...
              {selectedOrder.items?.map((item, index) => (
                <div key={index} className="orderItem">
                  ...
                  <div className="itemInfo">
                    ...
                    <p className="itemAuthor">por {item.author}</p>
            ...
            {selectedOrder.tracking?.history && (
              <div className="trackingHistory">
                ...
                <div className="timeline">
                  {selectedOrder.tracking.history.map((event, index) => (
                    <div key={index} className="timelineItem">
                      <div className="timelineDot"></div>
                      <div className="timelineContent">
                        <p className="timelineStatus">{event.status}</p>
                        <p className="timelineDate">{formatDate(event.date)}</p>
                        {event.location && (
                          <p className="timelineLocation">📍 {event.location}</p>
                        )}
                      </div>
                    </div>
                  ))}
